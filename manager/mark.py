...
def update_todo(id: str, status: str, config: dict):
	if id in config:
		config[id]["desc"] = status
...
def read_config(target: str, status: str):
	try:
		with open("config.json", "r") as config:
			config_parsed = json.load(config)

		update_todo(target, status, config_parsed)
...
def main(user_input):
	read_config(user_input.id, user_input.status4)
